...
export const execute = async (inter: ChatInputCommandInteraction) => {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      `<@${user.id}> has lost ${amount} social credits. Now they have ${
        existingUser.credits
      } social credits.${reason ? `\nReason: ${reason}` : ""}${
        addedRoles.length > 0
          ? `\nAdded Roles: ${addedRoles
              .map((addedRole) => `<@&${addedRole}>`)
              .join(", ")}.`
          : ""
      }${
        removedRoles.length > 0
          ? `\nRemoved Roles: ${removedRoles
              .map((removedRole) => `<@&${removedRole}>`)
              .join(", ")}.`
          : ""
      }`
    )
  ...
  const socialCreditImage = await generateSocialCreditImage(amount, "negative");
